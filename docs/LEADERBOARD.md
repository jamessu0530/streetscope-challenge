...
| `region` | `world` / `asia` 等 |
| `playedAt` | 創下該筆個人最佳紀錄的時間 |

### 唯一性與更新規則

- 每位玩家在每個 **`mode` + `region`** 組合只會有 **一筆** 紀錄（資料庫唯一索引）。
- 結算 POST 時：若新總分 **高於** 該組合的舊紀錄 → 更新分數與 `playedAt`；否則 **保留原最佳**，僅同步 `displayName`。
- GET 排行榜時：在目前的篩選條件下，**同一 `userId` 只顯示一筆**（取最高分；同分則較新者）。
...
- **個人最佳**：排行榜不會出現同一玩家重複多筆；只會看到該玩家在該模式／地區下的最高分。
